function [INF,SUP,MED,BAR] = VARirbandiv(VAR,VARopt,IVdata)
% =======================================================================
...
%
% I thank Alejandro Vicondoa for finding a bug in the defintion of
% bootstrapped residual and instruments.
...
%% STEP 1: choose the method and generate the residuals
    ...
    elseif strcmp(method,'wild')
        ...
        rr = 1-2*(rand(nobs,size(IVdata,2))>0.5);
        u = resid.*(rr*ones(size(IVdata,2),nvar));
        Z = [IVdata(1:nlag,:); IVdata(nlag+1:end,:).*rr];
    else
...
%% STEP 4: calculate "ndraws" impulse responses and store them
    ...
    [irf_draw, ~] = VARiriv(VAR_draw,VARopt,Z);  % uses options from VARopt, but parameters from VAR_draw
